...
label dialog_teacher_0_1:
    $ st = getChar('edumin')
    if st.sex == 'male':
        'Вы поговорили об успеваемости в школе, о двоечниках, особой темой для разговора был [st.name]. Его оценки оставляют желать лучшего, по сравнению со средним уровнем образования.'
    else:
        'Вы поговорили об успеваемости в школе, о двоечниках, особой темой для разговора был [st.name]. Её оценки оставляют желать лучшего, по сравнению со средним уровнем образования.'
    call screen show_stat
        
label dialog_teacher_0_2:
    $ st = getChar('corrmax')
    if st.sex == 'male':
        'Вы поговорили о поведении в школе, о хулиганах, особой темой для разговора был [st.name]. Его поведение оставляет желать лучшего'
    else:
        'Вы поговорили о поведении в школе, о хулиганах, особой темой для разговора была [st.name]. Её поведение оставляет желать лучшего.'
    call screen show_stat
    
label dialog_teacher_0_3:
    $ st = getChar('lustmax')
    if st.sex == 'male':
        'Вы поговорили о поведении в школе, о хулиганах, особой темой для разговора был [st.name]. Его поведение оставляет желать лучшего. Он попросту не пропускает ни одной юбки!'
    else:
        'Вы поговорили о поведении в школе, о хулиганах, особой темой для разговора была [st.name]. Её поведение оставляет желать лучшего. Такое чувство, как будто она охотится за всеми парнями в школе.'
    call screen show_stat
        
label dialog_teacher_0_4:
    $ rep = getPar(studs, 'rep')
    if rep < 20:
        'Вы поговорили о Вашей низкой репутации. Преподаватель посочувствовал Вам, пожелав удачи на новом месте работы.'
    elif rep < 50:
        'Вы поговорили о Вашей невысокой репутации. Преподаватель посоветовал Вам уделять больше внимания личному общению с учениками.'
    elif rep < 80:
        'Вы поговорили о Вашей высокой репутации. Преподаватель порадовался, что у них наконец-то такой уважаемый директор.'
    else:
        'Вы поговорили о Вашей непревзойдённой репутации. Преподаватель искренне завидует Вашему всеобщему уважению.'
    call screen show_stat
...
label dialog_teacher_0_6:
    if user.sex == 'male':
        '[user.name] 5 минут распинался про то, что не нужно химичке такое количество пробирок, колбочек, баночек и прочей фигни! Не нужно и всё! А у него мячей не хватает в спортзале. И одежды походу тоже.'
    else :
        '[user.name] хихикнув начала обсуждать нелёгкую жизнь Мустанговича. То ему мячей не хватает, то спортивной одежды...'
    call screen show_stat